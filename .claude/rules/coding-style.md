...
- 도메인 모델이 불가능한 상태를 허용하지 않는가?
- 도메인 모델이 데이터뿐 아니라 행위(behavior)를 포함하는가?
...
- usecase가 infra 구현체를 직접 참조하지 않고 domain trait에 의존하는가?
...
- 함수 역할이 명확한가?
- nesting depth가 낮은가?
- 명확성이 cleverness·brevity보다 우선하는가?
- 변수 scope가 작은가?
...
- business logic이 framework concern 없이 독립적으로 존재하는가?
- stable boundary가 존재하는가?
- concern separation이 되는가?
- 변경이 레이어 경계를 넘어 전파되지 않는가?
- 리팩토링 가능한 구조인가?
...
- validation·auth·logging 패턴이 프로젝트 전반에서 일관하게 유지되는가?
- 패턴 일관성이 cognitive load를 줄이는가?
...
- controller가 request parsing·validation·usecase 호출·response serialization만 담당하는가?
- usecase가 workflow ordering을 소유하는가?
- 하나의 usecase가 하나의 비즈니스 의도만 표현하는가?
...
- 식별자는 Newtype, 상태·분류값은 Enum으로 표현하는가?
- invalid state를 타입 시스템으로 제거하는가?
...
- async가 I/O 중심인가?
- 비동기 컨텍스트에서 `std::sync::Mutex` 대신 `tokio::sync::Mutex`를 사용하는가?
...
- repository interface가 domain 레이어에 정의되어 있는가?
- SQL concern이 infra 레이어에만 존재하는가?
...
- DTO가 분리되어 있는가?
- domain model을 직접 API response로 노출하지 않는가?
...
- `pub fn`/`pub struct`/`pub trait`에 `///` 주석이 있는가?
...
- request context에 사용자 정보가 주입되는가?
...
- tracing 필드 기반 형식(`key = value`)을 사용하는가?
- context-rich log를 남기는가?
- 에러가 삼켜지지 않고 로그에서 관찰 가능한가?
...
- mock보다 실제 domain object를 사용하는가?
- 레이어별 테스트 목적이 명확한가?
- 테스트 커버리지가 80% 이상인가?
...
- naming·folder 구조가 일관되어 예측 가능한가?
- layer boundary와 contract가 명시적으로 존재하는가?

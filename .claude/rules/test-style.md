...
name: 테스트 철학
description: >
  Classicist TDD 기반 공통 테스트 철학 — 모킹 경계, Assertion, 네이밍,
  피라미드, 엔티티 추출, Property-Based, Flaky, 마이그레이션, Rust 구현 가이드
---
...
**Classicist(Chicago) TDD를 선호한다. Mockist(London) 방식은 AI 기반 코드베이스에서 빠르게 테스트 신뢰도가 낮아진다.**
...
- **Fake** — 동작하는 간단한 구현(in-memory 등). 상태 기반으로 동작하고 검증도 상태로 한다
...
- **DB/ORM을 Mock으로 대체** — 인덱스, 외래키, 트랜잭션 격리 수준은 Mock으로 재현할 수 없다. Repository 테스트와 통합 테스트는 실제 DB(Testcontainers 등)를 사용한다
- 순수 함수와 유틸리티 — 실행 비용이 없으므로 대체할 이유가 없다
- 내가 소유한 Value Object, DTO, 도메인 엔티티 — 직접 생성해서 사용한다

단, 순수 서비스 로직 단위 테스트에서 Repository를 in-memory Fake로 대체하는 것은 허용된다. **"DB를 Mock하지 말라"**는 것이지, **"항상 실제 DB를 붙여야 한다"**는 뜻이 아니다. Mock은 DB 동작을 흉내만 내지만, Fake는 실제로 동작하기 때문에 상태 기반 검증이 가능하다.
...
HTTP 경계에서는 인터페이스 Mock보다 HTTP 레벨의 Fake 서버(Rust: `wiremock`, `httpmock` / JS: `msw`, `nock` 등)를 선호한다. 직렬화/역직렬화, HTTP 헤더 처리, 상태 코드 분기까지 검증 범위에 포함되기 때문이다.

**대체가 허용되는 기술적 기준:**

Mock보다 Fake를 먼저 고려하되, 다음 조건 중 하나에 해당할 때 내부 대체를 허용한다.

- **비결정성 제거** — 시스템 시계, 난수처럼 테스트를 불안정하게 만드는 요소
- **재현 불가능한 실패 시뮬레이션** — 실제 인프라에서 만들기 어려운 에러 케이스
- **피드백 루프 단축** — 순수 계산 로직 검증에서 DB 기동이 불필요한 경우

세 조건 중 어디에도 해당하지 않는 내부 대체는 테스트 범위를 좁힐 뿐, 신뢰도를 높이지 않는다.
...
fn create_order_fails_when_stock_is_zero() {
    ...
    // Act
    let result = create_order(&repo, product, Quantity::new(1));

    // Assert
    assert!(matches!(result, Err(OrderError::OutOfStock)));
}
```

세 단계가 명확히 구분되지 않는 테스트는 한 번에 여러 동작을 검증하거나 Arrange가 Act에 뒤섞인 것이다. 구조를 먼저 정리한다.
...
원칙은 **"상호작용 검증을 기본 수단으로 삼지 않는다"**이지, 상호작용 검증 자체를 배제하는 것이 아니다. 함수 내부를 리팩토링해서 DB 조회 횟수를 줄이거나 메서드 호출 순서를 바꾸면 호출 횟수·인자 검증 중심 테스트(`mockall::expect`, `toHaveBeenCalledWith`, `verify` 등)는 깨진다. 동작은 동일한데도 불구하고.
...
- 상호작용 검증(`mockall::expect`, `verify`, `toHaveBeenCalledWith`)은 보조 수단이다
...
```rust
// 비결정적 필드가 섞인 경우 — 의미 있는 필드만 검증
assert_eq!(result.name, "kim");
assert_eq!(result.status, UserStatus::Active);
// result.id, result.created_at 는 검증 생략
```
...
side-effect 자체가 요구사항일 때는 결과 상태만으로 검증하기 어렵다. 이메일 발송, 메시지 큐 발행, 외부 API 호출이 여기 해당한다. 이 경우 "발생했는가"라는 이벤트 자체가 비즈니스 요구사항이다. 또한 rate limit, retry 정책, idempotency key 전달처럼 호출 계약이 중요한 경우에도 상호작용 검증이 의미를 갖는다.

부작용 검증에는 두 가지 방법이 있다.
...
예를 들어 사용자 생성 기능을 테스트할 때, `user_repository.save()`가 호출됐는지를 검증하는 것은 구현 세부사항이다. 올바른 방향은 실제 DB에서 레코드를 조회해 저장됐는지 확인하는 것(상태), 그리고 가입 완료 이벤트가 발행됐는지 확인하는 것(상호작용)을 함께 검증하는 것이다.
...
```
user_repo_save_called_once()
calls_insert_then_publishes_notification()
should_work()
```
...
- 일회성 스크립트는 프로덕션 데이터에 영향을 미치지 않는 한 테스트하지 않는다
...
중요한 실행 원칙은 성능이다. 트랜잭션 롤백과 컨테이너 재사용을 전제로, 통합 테스트는 개발 피드백 루프를 방해하지 않는 수준으로 유지한다 (통합 테스트당 200ms 미만 목표). 전체 DB 초기화보다 트랜잭션 롤백을 우선하고, 병렬 실행 시 트랜잭션 격리가 테스트 간 공유 상태를 만들지 않는지 주의한다. 비용이 큰 외부 시스템 연동 테스트는 환경 변수 플래그(`LIVE_TEST=true`)로 격리해 일반 CI에서는 실행하지 않는다.
...
- 불변식(invariant)이 여러 서비스에 중복 체크되고 있다 (예: "energy는 0 미만 불가", "hunger는 0~100 범위")

같은 규칙이 세 군데에 흩어져 있다면 엔티티로 끌어올려 한 곳에서 보장해야 한다.

**Before — 서비스에 로직이 뒤섞인 상태:**

```rust
user.hunger -= EAT_AMOUNT * 2;
user.energy += SLEEP_AMOUNT * 2;
...
`user.eat()`는 이제 순수 메모리 기반 단위 테스트가 된다. DB 없이 밀리초 단위로 실행되고, 모킹도 필요 없다. 도메인 로직을 엔티티에 캡슐화하면 테스트 속도가 빨라지고, 로직이 여러 서비스에 중복되는 문제도 함께 해결된다. 엔티티 추출의 가장 직접적인 효과는 단위 테스트를 가능하게 만드는 구조 자체를 확보하는 것이다.

이 원칙은 DDD(도메인 주도 설계)의 Rich Domain Model과 맞닿아 있다. 빈약한 도메인 모델(Anemic Domain Model)이 서비스 비대화와 테스트 복잡도 상승을 동시에 유발한다는 Martin Fowler의 지적은 유효하다. 다만 Rich Domain Model이 항상 더 좋은 것은 아니다. CRUD 중심의 단순한 시스템에서는 Anemic 모델이 더 단순하고 적합하다. Rich Domain Model이 효과를 발휘하는 것은 도메인 복잡도가 일정 수준 이상일 때다.
...
대표 라이브러리: `proptest` (Rust), `quickcheck` (Rust), `fast-check` (TypeScript), `hypothesis` (Python)
...
테스트가 실패할 때 단순히 "실패한 입력"을 보여주는 게 아니라, 실패를 재현하는 **최소 반례(minimal failing case)**를 자동으로 찾아준다. 수천 개의 무작위 입력 중 하나가 실패했을 때 원인을 직접 추적하는 것은 어렵지만, shrinking이 적용된 도구는 그 입력을 최대한 단순화해서 돌려준다. `proptest`와 `fast-check`는 기본 지원하고, `hypothesis`는 자동 shrinking을 포함한다.
...
## 7. Flaky 테스트 처리 — 격리와 근본 원인 제거

불안정한(flaky) 테스트는 팀의 신뢰를 갉아먹는다. CI가 무작위로 빨간불을 켜면 개발자들은 CI 결과를 무시하기 시작하고, 그 순간부터 테스트 스위트는 존재 의미를 잃는다.

**원칙:**

1. 불안정한 테스트는 커밋하지 않는다. 이미 들어왔다면 24시간 이내에 격리한다. 탐지 → 분류 → 격리의 흐름을 명확히 하고, 동일 테스트가 2회 이상 실패하면 머지를 차단하는 기준을 CI에 정의해둔다.
...
AI 도구가 생성한 코드는 비동기 흐름과 숨은 상태 의존성이 예상보다 많다. 이 두 가지는 flaky의 가장 흔한 원인이기도 하다. AI 코드베이스에서 flaky 테스트가 늘어난다면, 생성된 코드가 외부 상태에 암묵적으로 의존하고 있다는 신호로 읽는 것이 맞다.
...
3. **최악의 파일부터** — 상호작용 검증(`mockall::expect`, `toHaveBeenCalledWith` 등) 위주의 테스트가 가장 많은, 변경 빈도가 높고 버그가 잦은, 자주 flaky가 발생하는, 핵심 비즈니스 로직을 담은 파일 3~5개를 먼저 식별해 한 도메인씩 재작성한다.
4. **실제 DB를 점진적으로 도입** — 시작 전에 현재 테스트 실행 시간, flaky 비율, CI 소요 시간을 측정해 baseline을 확보한다. 스키마가 가장 단순하거나 버그 발생률이 가장 높은 도메인 하나부터 시작해, 속도(<200ms/테스트), 결정성, CI 안정성을 검증한다. baseline 대비 개선이 확인된 후에만 다른 도메인으로 확장한다.
...
테스트 실행 시간이 상대적으로 2배 이상 늘거나, 전체 테스트가 5분 또는 PR 단위 테스트가 1분을 초과하면 진행을 멈추고 최적화(트랜잭션 롤백, 컨테이너 재사용)를 먼저 적용한다. 성능 문제를 무시하면 팀 전체가 결국 테스트 실행을 기피하게 된다.

**마이그레이션 성공 지표:**

외부 시스템 전환 시 계약 테스트(`httpmock`, `wiremock` 등)를 추가해 외부 API 계약이 깨지는 것을 조기에 탐지한다. 마이그레이션이 실제로 개선되고 있는지는 지표로 판단한다. flaky 비율 감소, 테스트 실패 시 버그 재현률 증가, CI 안정성 향상이 기준이다. 이 지표가 없으면 "좋아졌다"는 느낌으로만 판단하게 되고, 방향 수정이 늦어진다.
...
- 기존 도구로 해결 가능한데 새 모킹 프레임워크를 추가 (Rust: `mockall` 도입 시 경계 정당성 필수)
- assertion이 없는 테스트 — CI를 통과하지만 아무것도 검증하지 않는다
- 의미 없는 assertion(`assert!(result.is_some())`, `assert!(items.len() > 0)` 등 존재만 확인하는 형태) — 테스트는 존재하지만 실제 버그를 잡지 못한다
...
공통 철학을 Rust 코드베이스에 적용할 때의 구체적인 지침이다.

### 테스트 배치

- **단위 테스트** — 해당 소스 파일 말미에 `#[cfg(test)]` 블록으로 배치
- **통합/DB/HTTP API 테스트** — `{crate}/tests/` 디렉터리에 별도 파일로 분리

```rust
// src/domain/user.rs 내부 단위 테스트
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn eat_reduces_hunger_when_called() {
        let mut user = User::new(100, 50);
        user.eat();
        assert_eq!(user.hunger(), 80);
    }
}
```
...
tokio 런타임이 필요한 테스트는 `#[tokio::test]`를 사용한다.

```rust
#[tokio::test]
async fn create_user_succeeds_when_input_is_valid() {
    let repo = InMemoryUserRepository::new();
    let user = create_user(&repo, valid_input()).await.unwrap();
...
동일한 검증 로직을 다양한 입력으로 반복할 때 `rstest`의 `#[case]`를 사용한다. Property-Based Testing과 달리 개발자가 케이스를 직접 정의한다.
...
#[rstest]
#[case("", OrderError::EmptyTitle)]
#[case("a".repeat(201), OrderError::TitleTooLong)]
#[case("   ", OrderError::BlankTitle)]
fn create_order_fails_when_title_is_invalid(
    #[case] title: &str,
    #[case] expected: OrderError,
) {
    let result = create_order(title, valid_quantity());
    assert_eq!(result.unwrap_err(), expected);
}
```

같은 검증 로직을 독립 테스트로 반복 작성하고 있다면 `#[case]`로 통합한다. 단, 케이스마다 맥락 설명이 별도로 필요하다면 각각 독립 테스트로 유지하는 것이 낫다.

**`#[case]` vs `proptest` 선택 기준:**

| 상황 | 선택 |
|---|---|
| 케이스를 직접 열거할 수 있고 유한하다 | `#[case]` |
| 불변 조건이 명확하고 경계값을 자동 탐색해야 한다 | `proptest` |

### DB 격리 — 트랜잭션 롤백

sqlx 기반 통합 테스트는 `#[sqlx::test]` 매크로를 사용한다. 테스트마다 독립된 트랜잭션이 열리고 종료 후 자동 롤백되어 DB 상태를 초기화한다.

```rust
// tests/user_repository_test.rs
#[sqlx::test]
async fn find_user_returns_none_when_not_found(pool: PgPool) {
...
전체 DB 초기화보다 트랜잭션 롤백을 우선한다. 초기화는 테스트 간 의존성을 만들고 병렬 실행을 방해한다.

### 외부 HTTP 서비스 모킹

| 도구 | 특징 | 적합한 상황 |
|---|---|---|
| `wiremock` | 선언형 HTTP stub, 요청 매칭·검증 포함 | 복잡한 외부 API 계약 검증 |
| `httpmock` | 경량 HTTP mock 서버, 설정 간단 | 빠른 외부 호출 대체 |
...
#[tokio::test]
async fn notify_sends_request_when_order_completed() {
    let mock_server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/notify"))
        .respond_with(ResponseTemplate::new(200))
        .expect(1)  // MockServer drop 시 호출 횟수를 자동 검증
        .mount(&mock_server)
        .await;

    send_notification(&mock_server.uri(), order_id()).await.unwrap();
    // mock_server가 스코프를 벗어날 때 expect(1) 조건이 자동으로 검증된다
}
```
...
```bash
cargo test --all             단위 + 통합 테스트
cargo tarpaulin --out Html   # 목표: 80% 이상
cargo bench (선택)           성능 회귀 확인
```

커버리지 숫자보다 **어떤 경로가 빠졌는가**가 중요하다. 인증, 결제, 권한 경로의 미커버리지는 반드시 해소한다.

...
- **security.md 보완 적용** — R-04·R-05·R-09 판단 시 보안 규칙을 추가 기준으로 적용
...
4. `.claude/rules/test-style.md` — R-08 보완 기준
...
### test-style.md → R-08 보완 기준

**R-08 테스트** 판단 시 test-style.md 전체 추가 적용:
- test-style.md §4. 테스트 피라미드: 전체 80%+ 목표 기준으로 테스트 부족 여부 판단
- test-style.md §4. 테스트 피라미드: `Result` 반환 함수에 에러 케이스 테스트 없으면 ⚠️ Recommended (High)
- test-style.md §3. 테스트 네이밍: `test1()`, `test_order()` 등 의미 없는 이름이면 💡 Suggestions (Low)
- test-style.md §9. PR Red Flags: `#[ignore]` 담당자·기한 없이 추가, 상호작용 검증만 있으면 ⚠️ Recommended (Medium)
- test-style.md §10. Rust 테스트 구현 가이드: `#[should_panic]` 사용 시 ⚠️ Recommended (Medium) (Result 반환 방식 권고)
...
📏 보완:   [해당 시 — security.md §섹션 또는 test-style.md §섹션]
...
PR 체크리스트:
  ...
  □ 에러 케이스 테스트 존재 (test-style.md §4. 테스트 피라미드)
...
> 보완 기준: .claude/rules/security.md · .claude/rules/test-style.md
> 리뷰 대상: [파일 목록]
...
**[R-XX] [제목]** — `[파일명:행번호]`
[설명] / 근거: [security.md §섹션 또는 test-style.md §섹션]
...
|------|--------|------|-----------|-----------|
| [R-XX] | 🔴 | [파일:행] | [내용] | [security/test-style.md 해당 시] |
...
| **R-04** | 에러 처리 | §5.3, §5.4, §1.2 | `unwrap()`, `panic!` in lib, 문자열 에러, 내부 정보 노출 | 🚫⚠️ | security.md §에러 응답 |
...
| **R-08** | 테스트 | §6 전체 | 에러 케이스 없음, 도메인 행동 미검증, 구현 세부 검증 | ⚠️💡 | **test-style.md 전체** |
...
🚫 테스트 삭제 또는 비활성화 (test-style.md §9. PR Red Flags 참조)

...
triggers:
  ...
  - /code-review-feedback-rust --pr [PR번호]
  - /code-review-feedback-rust --staged
  - /code-review-feedback-rust --commit [커밋해시]
  - /code-review-feedback-rust --branch
  - /code-review-feedback-rust --branch [브랜치명]
  - /code-review-feedback-rust [파일경로 또는 모듈명]
  - /code-review-feedback-rust --scope [카테고리 키워드]
  - /code-review-feedback-rust --severity [critical|high|medium|low]
  - /code-review-feedback-rust --event [comment|request-changes|approve]
  - /code-review-feedback-rust --dry-run
  - /code-review-feedback-rust --catalog
  - /code-review-feedback-rust --help
reference: ../code-review-rust/CODE_REVIEW_RUST.md
rules:
  - ../../rules/security.md
  - ../../rules/test.md
---
...
/code-review-feedback-rust --severity critical       Critical 이슈만
/code-review-feedback-rust --severity high           High 이상만
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  #   종류      위치                  심각도    제목
  ─── ──────── ──────────────────── ──────── ────────────────────────
  1   인라인   src/handler.rs:42     🔴 Critical  unwrap() 사용
  2   인라인   src/handler.rs:87     🟠 High      std::Mutex in async
  3   인라인   src/repo.rs:15        🟡 Medium    clone() 불필요
  4   전체     (리뷰 본문)           🔵 Low       문서화 누락
...
⚠️  심각도: 🔴 Critical  [C-CR-01]

> [문제 코드 1~3줄]

**unwrap() 사용으로 인한 패닉 위험**
...
**`approve` 사용 조건**: Critical / High 이슈가 0건인 경우에만 허용.
Critical / High가 있으면 자동으로 `COMMENT`로 강등하고 경고를 출력한다.
...
- `--event approve`는 Critical + High 이슈가 0건일 때만 허용한다.

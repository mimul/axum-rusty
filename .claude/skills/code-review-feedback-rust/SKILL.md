...
description: >
  ...
  /code-review-rust와 동일한 9개 카테고리(R-01~R-09) 분석을 수행하지만, 소스 코드를 직접 수정하지 않고 GitHub PR에 리뷰 코멘트를 게시한다.
...
# 필터 옵션 (모든 모드와 조합 가능)
/code-review-feedback-rust --scope domain            R-01 도메인 모델만
/code-review-feedback-rust --scope state             R-02 상태 & 모델링만
/code-review-feedback-rust --scope boundary          R-03 경계 조건만
/code-review-feedback-rust --scope error             R-04 에러 처리만
/code-review-feedback-rust --scope ownership         R-05 소유권 & 메모리만
/code-review-feedback-rust --scope control           R-06 제어 흐름만
/code-review-feedback-rust --scope abstraction       R-07 추상화 & trait만
/code-review-feedback-rust --scope test              R-08 테스트만
/code-review-feedback-rust --scope security          R-09 보안 (rust-security-style.md 전체 기준 집중 분석)
...
rules 로드 순서·R-01~R-09 rust-coding-style.md 매핑 테이블·§9 안티 패턴 횡단 체크·rust-security-style.md 보완 기준·rust-test-style.md 보완 기준은 `../code-review-rust/SKILL.md`의 STEP 2를 그대로 따른다.
...
📝 종합 평가
  [설계 방향, 잠재 리스크, rust-coding-style.md 철학 관점 개선 제안 3~5줄]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
🏷️  분류:   🚫 Blocking  [R-04]
📐 근거:   rust-coding-style.md 경계조건(unwrap 금지) 처리 방식
...
📐 rust-coding-style.md 경계조건(unwrap 금지): 에러는 무시하지 않고 명확하게 전달하거나 처리한다
🔒 보완: rust-security-style.md §5 에러 처리와 정보 노출 (보안 관련 시)
───────────────────────────────────────
...
> 1차 기준: rust-coding-style.md (도메인 중심 진화형 코딩 원칙)
> 보완 기준: rust-security-style.md · rust-test-style.md

### 🚫 Blocking Issues
- **[R-XX]** `[파일:행]` [이슈 제목]: [설명] / 📐 rust-coding-style.md §[섹션]
...
**대상**: [파일 / 브랜치 / 커밋]
**분석 기준**: CODE_REVIEW_RUST.md R-01~R-09
**1차 기준**: rust-coding-style.md (도메인 중심 진화형 코딩 원칙)
**보완 기준**: rust-security-style.md · rust-test-style.md
...
| 파일 | 용도 | 로드 시점 |
|------|------|-----------|
| `../code-review-rust/CODE_REVIEW_RUST.md` | R-01~R-09 체크리스트 | 스킬 실행 시 항상 |
| `../../rules/rust-coding-style.md` | 도메인 중심 코딩 원칙 — R-01~R-09 **1차 판단 기준** | **STEP 2 분석 시작 전 로드 (1순위)** |
| `../../rules/rust-security-style.md` | 보안 규칙 (공통) — R-04·R-05·R-09 보완 기준 | **STEP 2 분석 시작 전 로드 (2순위)** |
| `../../rules/rust-security-style.md` | 보안 규칙 (Rust 전용) — R-04·R-05·R-09 Rust 특화 보완 | **STEP 2 분석 시작 전 로드 (3순위)** |
| `../../rules/rust-test-style.md` | 테스트 규칙 — R-08 보완 기준 | **STEP 2 분석 시작 전 로드 (4순위)** |

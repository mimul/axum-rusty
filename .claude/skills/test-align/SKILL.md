...
description: |
  ...
  실행 흐름 (STEP 0 → 6):
    ...
    STEP 3  테스트 작성 계획 수립 → 👤 사용자 승인
    STEP 4  테스트 코드 제시 → 👤 사용자 확인 후 파일 저장 + cargo test + 커밋 (항목별 반복)
    STEP 5-0 커버리지 게이트 (cargo tarpaulin, 80% 미만이면 PR 차단)
    STEP 5  완료 요약 출력
    STEP 6  PR 초안 제시 → 👤 사용자 승인 → push + gh pr create

  핵심 제약:
    - §17.1 Blocking 갭(Mock DB·assertion 없는 테스트·sleep 기반 flaky 등) 즉시 차단
    - 커버리지 80% 미달 시 PR 생성 금지
    - 사용자 확인(👤) 전 파일 저장·push 절대 금지
---
...
- **보여주고 확인받기** — 코드를 먼저 제시, 사용자 승인 후에만 파일 저장
...
> 👤 = 사용자 확인 대기 단계 (STEP 3 계획 승인 · STEP 4 항목별 저장 확인 · STEP 6 PR 생성 확인)
...
- 테스트 코드 내 `tokio::time::sleep` · 공유 전역 상태(`static`, `OnceCell`) 패턴 (§8)
- `benches/` 디렉토리 및 criterion 사용 여부 (§12.1)
- proptest 의존성 및 `proptest!` 매크로 사용 여부 (§5)
- 로직 없는 getter/setter·순수 CRUD 테스트 존재 여부 (§16)

---

# STEP 2 — rules 로드 및 갭 분석 리포트
...
| §12.1 | auth middleware·DB hotspot·직렬화 등 핵심 경로에 benchmark 없음 |
...
| §7.1 | AAA 패턴 미준수 테스트 |
| §10 | 긴 Mock 체인(`expect` 호출 5개 이상) → Integration 테스트 전환 권고 |
| §14 | proptest·wiremock-rs·sqlx::test 등 권장 도구 미사용 |
| §16 | 로직 없는 getter/setter·순수 CRUD·프레임워크 배선 테스트 → 삭제 권고 |

...
description: >
  ...
  rules/test-style.md를 테스트 철학·규칙의 권위 문서로 사용한다.
---
...
2. `../../rules/test-style.md` — 테스트 철학·Mocking·Naming·PR 기준 (권위 문서)
...
| test-style.md 섹션 | 갭 분석 적용 |
|---|---|
| 테스트 철학 | Classicist 접근, 통합 테스트 우선 여부 확인 |
| §1. 모킹 경계 | 내부 mock(mockall 등) 사용 여부 → 발견 시 보고 |
| §2. Assertion 스타일 | 상호작용 검증만 있는 테스트 → 상태 검증으로 교체 권고 |
| §3. 테스트 네이밍 | 기존 테스트명 검증, 위반 시 보고 |
| §4. 테스트 피라미드 | 레이어별 테스트 예산 준수 여부, 단순 CRUD 생략 기준 |
| §9. PR Red Flags | mockall 내부 사용, 스냅샷 비결정값, #[ignore] 무단 사용 등 |
| §6. Property-Based Testing | 4번째 예제 테스트 → proptest 전환 권고 |
...
⚠️ 개선 권고 (rules/test-style.md 위반)
...
📏 규칙:       [test-style.md §섹션명, security.md §섹션명 해당 시]
...
PR 체크리스트:
  ...
  □ 테스트명 네이밍 규칙 준수 (rules/test-style.md §3. 테스트 네이밍)
  □ 테스트에 하드코딩 시크릿 없음 (security.md §7 + security-rust.md §9)
  □ PR Red Flags 없음 (rules/test-style.md §9. PR Red Flags)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
## 금지 사항 (`rules/test-style.md §9. PR Red Flags` 전체 적용)

```
🚫 내부 모듈(Repository, Usecase) mock — 실제 DB 사용 (test-style.md §1. 모킹 경계)
...
🚫 sleep() 사용 — 근본 원인 해결 (test-style.md §7. Flaky 테스트 처리)
...
| `../../rules/test-style.md` | 테스트 철학·Mocking·Naming·PR 기준 (권위 문서) | **STEP 2 분석 시작 전 로드** |

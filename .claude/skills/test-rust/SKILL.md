...
**파일/모듈명을 인수로 지정한 경우** — 분석 전 먼저 존재 여부를 확인한다:

```bash
ls [지정된 경로]
```

파일이 없으면 Claude가 아래 명령을 실행하여 후보 목록을 수집한 뒤 출력하고
**즉시 종료**한다. 테스트를 작성하지 않는다:
...
단, T-T-02~04가 T-T-06(공통 헬퍼)에 의존하는 경우 T-T-06을 자동으로 포함하고
"(--type 필터에 의해 자동 포함)" 표기한다.

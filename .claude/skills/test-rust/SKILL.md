...
**파일/모듈명을 인수로 지정한 경우의 선행 확인 (브랜치 생성 전):**
브랜치를 생성하기 전에 지정 파일의 존재 여부를 먼저 확인한다.
파일이 없으면 브랜치를 생성하지 않고, 즉시 STEP 1의 오류 출력 형식으로 종료한다.
...
2. `../../rules/test.md` — 테스트 철학·Mocking·Naming·PR 기준 (권위 문서)
...
| 검사 항목 | 근거 |
|-----------|------|
| 테스트 픽스처에 실제 토큰·비밀번호 하드코딩 없음 | security.md §7 + security-rust.md §9 |
| Newtype 생성 시 잘못된 입력 케이스 포함 여부 | security.md §2 + security-rust.md §5 |
| 에러 응답이 내부 정보를 노출하지 않는지 검증 케이스 포함 여부 | security.md §5 + security-rust.md §6 |
| `unwrap()`/`expect()` 사용 함수에 패닉 유발 경계 케이스 포함 여부 | security-rust.md §3 |
| 역직렬화 대상 타입에 잘못된 입력 케이스 포함 여부 | security-rust.md §7 |
...
**갭이 0건인 경우** — 아래 형식으로 출력하고 STEP 5로 바로 이동한다:
...
다음 단계:
  ...
  open coverage/tarpaulin-report.html   # 현재 커버리지 확인
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
PR 체크리스트:
  ...
  □ 테스트에 하드코딩 시크릿 없음 (security.md §7 + security-rust.md §9)
...
🚫 테스트에 하드코딩된 시크릿 (security.md §7 + security-rust.md §9)

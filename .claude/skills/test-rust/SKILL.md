...
|---|---|
| §13.1, §4.3 | 통합 테스트에서 Mock DB / Mock Repository 발견 → 즉시 보고 |
| §13.1, §5.2 | 상호작용 검증만 있고 상태 검증 없는 테스트 → 즉시 보고 |
| §13.1, §10.2 | `SystemTime::now()` / 시드 없는 난수 등 비결정적 출력 고정 사용 → 즉시 보고 |
| §13.1, §10.3 | 이슈 링크·담당자·기한 없이 단순 `#[ignore]` → 즉시 보고 |
...
| rust-test-style.md 섹션 | 갭 분석 적용 |
|---|---|
| §6.3 | 핵심 비즈니스 로직(인증·권한·결제·상태 전환) 테스트 없음 → 반드시 작성 대상 포함 |
| §6.3 | 복잡한 분기·상태 전환 로직 테스트 없음 → 반드시 작성 대상 포함 |
| §13.2 | `mockall expect` 호출이 실제 assert보다 압도적으로 많음 → Classicist 전환 권고 |
...
| rust-test-style.md 섹션 | 갭 분석 적용 |
|---|---|
| §4 | 내부 mock(mockall 등) 사용 여부 → 발견 시 Fake/실제 DB 전환 권고 |
| §3.2 | 기존 테스트명이 `<동작>_<예상_결과>_when_<조건>` 템플릿 미준수 → 개선 권고 |
...
| rust-test-style.md 섹션 | 갭 분석 적용 |
|---|---|
| §9.1 | 동일 함수에 예시 테스트 4개 이상 작성 시 proptest 전환 권고 |
| §11.1 | 설정·모킹 코드 과잉 시 Builder 패턴 도입 권고 |
...
| `../../rules/rust-security-style.md` | 보안 규칙 §1~§12 | **STEP 2 분석 시작 전 로드** |

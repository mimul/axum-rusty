...
테스트 철학·Mocking 규칙·네이밍·PR 기준은 `rules/test.md`가 권위 문서다.
...
> 내부 모듈(Repository, Usecase)을 mock하는 것은 `rules/test.md §Mocking Rules`에서 금지한다.
...
### 작성 기준 (`rules/test.md §When NOT to Write a Test` 준수)
...
> **Mock 금지**: `rules/test.md §Mocking Rules` — DB/ORM·내부 Repository는 mock하지 않는다.
...
> **작성 기준**: `rules/test.md §Property-Based Testing` 참조.

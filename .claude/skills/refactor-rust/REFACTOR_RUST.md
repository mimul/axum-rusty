...
| 원칙 | 설명 | rust-coding-style.md 근거 |
|------|------|---------------------------|
| **기능 동치(Behavioral Equivalence)** | 외부에서 관찰 가능한 동작은 100% 동일하게 유지 | 핵심원칙§4 소규모 변경 |
| **도메인 중심(Domain-Centric)** | 리팩토링 결과물은 도메인 개념이 코드에 더 잘 드러나야 한다 | 핵심원칙§3, 설계§2 |
| **의도 드러내기(Intent-Revealing)** | 변수·함수·타입명은 구현 방식이 아닌 의도를 표현해야 한다 | 핵심원칙§2, 네이밍 섹션 |
| **소규모 증분(Small Increments)** | 한 번에 하나의 변환만 수행한다 | 핵심원칙§4 소규모 변경 |
| **항상 그린(Always Green)** | 매 단계마다 `cargo test` 통과 확인 | 리팩토링은 개발의 리듬이다 |
| **컴파일러를 동반자로(Compiler as Partner)** | 컴파일 오류 메시지를 설계 가이드로 활용 | Rust-specific |
| **되돌릴 수 있게(Reversible)** | Git 커밋 단위를 작게 유지 | 핵심원칙§4 소규모 변경 |

---

## 리팩토링 트리거

다음 상황 중 하나라도 해당되면 리팩토링을 시작한다 (rust-coding-style.md §3.2):
...
| 냄새 유형 | 증상 | rust-coding-style.md | Clippy Lint |
|-----------|------|----------------------|-------------|
| 의미 없는 이름 | `data`, `util`, 매직 넘버 | 핵심원칙§2, 네이밍 섹션 | `clippy::unreadable_literal` |
| Primitive 집착 | `u64`/`String`으로 도메인 개념 표현 | 핵심원칙§3, 설계§2 | 수동 검토 |
| 빈약한 도메인 모델 | 도메인 로직이 서비스에만 있고 엔티티는 데이터만 보유 | 설계§2 | 수동 검토 |
| Bool 플래그 남용 | `is_paid + is_shipped` 조합 상태 | 설계§1(Tell Don't Ask), 경계조건(Invalid State) | 수동 검토 |
| 과도한 중첩 | match/if let 3단계 이상 | 설계§1(Early Return) | `clippy::collapsible_match` |
| 거대 함수 | 함수 50줄 초과 | 설계§1 | `clippy::too_many_lines` |
| 중복 코드 | 동일 패턴 3회 이상 반복 | 설계§3(YAGNI) | 수동 검토 |
| 암묵적 경계 처리 | `.unwrap()`, 인덱스 무방비 접근 | 경계조건(unwrap 금지) | `clippy::unwrap_used` |
| Clone 남용 | `.clone()`이 컴파일 오류 회피용 | 핵심원칙§1, 성능 섹션 | `clippy::clone_on_ref_ptr` |
| 도메인 없는 util | `utils.rs`에 비즈니스 로직 혼재 | 핵심원칙§3 | 수동 검토 |
| 불필요한 힙 할당 | `String` vs `&str` 혼용 | 핵심원칙§1, 성능 섹션 | `clippy::box_default` |
| 블로킹 I/O | async 컨텍스트에서 blocking 호출 | 성능 섹션(tokio::Mutex) | `clippy::async_yields_async` |
...
> 각 항목은 `rust-coding-style.md`의 해당 섹션에서 직접 도출했다.

| 코드 | 이름 | rust-coding-style.md 근거 | 우선순위 |
|------|------|---------------------------|----------|
| R-R-01 | 의도를 드러내는 네이밍 | 핵심원칙§2, 네이밍 섹션 | ⚠️💡 |
| R-R-02 | 빈약한 도메인 모델 개선 | 핵심원칙§3, 설계§2 | ⚠️💡 |
| R-R-03 | 상태 & 제어 흐름 명확화 | 설계§1(Tell Don't Ask·Early Return), 설계§2, 경계조건(Invalid State) | 🚫⚠️ |
| R-R-04 | 함수 분해 & 단일 책임 | 설계§1(작고 조합 가능한 구조), 핵심원칙§4 | ⚠️💡 |
| R-R-05 | 중복 제거 & 적시 추상화 | 설계§3(YAGNI), 설계§5(impl Trait), 핵심원칙§1 | ⚠️💡 |
| R-R-06 | 경계 조건 & 에러 처리 명시화 | 경계 조건은 도메인의 일부다(전체), 핵심원칙§2 | 🚫⚠️ |
| R-R-07 | 소유권 & 변경 용이성 | 핵심원칙§1, 설계§1, 성능 섹션(N+1·clone·tokio::Mutex) | ⚠️💡 |
| R-R-08 | 모듈 구조 도메인화 | 핵심원칙§3, 설계§4(접근권한 최소화), 설계§1 | 💡 |

---

### [R-R-01] 의도를 드러내는 네이밍

**rust-coding-style.md 근거**: §1.2 의도를 드러내는 코드, §4 네이밍 규칙
...
**네이밍 금지 목록** (rust-coding-style.md §4.2):
```
...
**rust-coding-style.md 근거**: §1.3 도메인 중심 설계, §2.2 도메인 모델
...
// ✅ After-B: 풍부한 도메인 모델 — 데이터와 행동이 함께 (rust-coding-style.md §2.2)
pub struct Order {
...
**rust-coding-style.md 근거**: §2.4 제어 흐름, §2.2 도메인 모델
...
// ✅ After-B: Early Return — 흐름이 선형 (rust-coding-style.md §2.1)
fn process(order: &Order) -> Result<Receipt, Error> {
...
**rust-coding-style.md 근거**: §2.1 구조, §1.4 점진적 진화
...
**rust-coding-style.md 근거**: §2.3 추상화, §1.1 변화 용이성 우선
...
// ✅ After-B: 추상화 제거, 직접 구현 (YAGNI, rust-coding-style.md §2.3)
fn format_as_json(data: &str) -> String { ... }
// Trait이 필요한 두 번째 구현체가 생기면 그때 Trait 도입
```

> **규칙**: 추상화는 **최소 3번의 반복 이후**에 도입한다. 코드가 두 곳에 있으면 복사를 유지한다. 세 곳이 되면 그때 추상화한다 (rust-coding-style.md §1.1).

---

### [R-R-06] 경계 조건 & 에러 처리 명시화

**rust-coding-style.md 근거**: §5 에지 케이스 & 경계 조건, §1.2 의도를 드러내는 코드

**적용 시점**: `.unwrap()` / 인덱스 직접 접근 / `unwrap_or_default()`로 경계 조건을 암묵적으로 처리할 때

```rust
// ❌ Before: 침묵하는 실패·암묵적 경계 처리 (rust-coding-style.md §5.4)
...
// ✅ After: 경계 조건을 도메인 로직으로 명시 (rust-coding-style.md §5.2)
...
**rust-coding-style.md 근거**: §1.1 변화 용이성 우선, §2.1 구조
...
**rust-coding-style.md 근거**: §1.3 도메인 중심 설계, §2.1 구조
...
// 공개 범위 원칙 (rust-coding-style.md §2.1):
// pub(super) ← 부모 모듈만 접근
...
❌ 기능 추가와 리팩토링 동시 진행 (rust-coding-style.md §9)
   → 한 PR은 하나의 의도만 담는다.
```
...
| 코딩 스타일 | `.claude/rules/rust-coding-style.md` |

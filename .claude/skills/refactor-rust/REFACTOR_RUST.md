...
Rust 특유의 리팩토링 카탈로그(R-R-01~R-R-10) 이전에, 언어에 무관하게 적용되는 리팩토링 원칙을 먼저 확인한다.
...
| **⚠️ 권장 수정** | 리팩토링 목적 미달성 (중복 잔존, SRP 위반) | 가능하면 이번 PR에 반영 |
| **💡 제안** | 추가 개선 기회 | 향후 고려 |
...
> 커버리지·테스트 원칙은 공통 리뷰 항목 — 테스트를 참조한다
...
> 기능 동치·에러·로그 동작 불변 원칙은 공통 리뷰 항목 — 기능 동치 검증을 참조한다
```
...
> 단계적 변환·기능 분리 원칙은 공통 리뷰 항목 — 단계적 변환을 참조한다
```

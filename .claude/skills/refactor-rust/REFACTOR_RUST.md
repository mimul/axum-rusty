...
| 원칙 | 설명 |
|------|------|
| **기능 동치(Behavioral Equivalence)** | 외부에서 관찰 가능한 동작은 100% 동일하게 유지 |
| **도메인 중심(Domain-Centric)** | 리팩토링 결과물은 도메인 개념이 코드에 더 잘 드러나야 한다 |
| **의도 드러내기(Intent-Revealing)** | 변수·함수·타입명은 구현 방식이 아닌 의도를 표현해야 한다 |
| **소규모 증분(Small Increments)** | 한 번에 하나의 변환만 수행한다 |
...
Rust 전용 카탈로그(R-R-01~R-R-10)에 앞서 언어에 무관하게 적용되는 원칙을 먼저 확인한다.

### 기능 동치 검증

- **외부 동작 불변**: 리팩토링 전후로 외부에서 관찰 가능한 동작이 100% 동일한가?
- **공개 API 불변**: 공개 함수·타입 시그니처가 변경되지 않았는가? (파괴적 변경은 별도 PR)
- **에러·로그 동작 불변**: 에러 메시지·코드·로그 포맷·환경 변수 키가 변경되지 않았는가?

### 코드 품질 향상 확인

- **SRP**: 리팩토링 후 각 모듈·함수가 하나의 명확한 책임을 가지는가?
- **DRY**: 중복 코드가 실제로 줄었는가? 유사한 로직이 2곳 이상 잔존하지 않는가?
- **KISS**: 불필요한 복잡성이 도입되지 않았는가? 리팩토링 이전보다 더 단순해졌는가?

### 가독성

- 변수·함수·타입명이 도메인 의도를 더 명확히 전달하는가?
- 함수 길이가 적절히 유지되는가? (기준: 50줄 이하)
- 매직 넘버·문자열이 의미 있는 상수로 추출되었는가?

### 테스트

- 전체 테스트가 통과하는가?
- 테스트 커버리지 기준선(80%)이 유지 또는 향상되었는가?
- 리팩토링 중 테스트를 수정했다면, 동작 변경이 아닌 구조 변경에 한정되는가?

### 단계적 변환

- 한 번에 하나의 변환만 수행했는가?
- 기능 추가와 리팩토링이 동시에 진행되지 않았는가?
- 각 변환이 독립적으로 커밋되어 되돌릴 수 있는가?
...
2. **냄새 유형**: 어떤 카탈로그 항목에 해당하는가? (예: `R-R-01 Clone 남용`)
...
| Primitive 집착 | `u64`/`String`으로 도메인 개념 표현 | 수동 검토 |
...
| Bool 플래그 남용 | `is_paid: bool`, `is_shipped: bool` 조합 상태 | 수동 검토 |
| 도메인 없는 util | `utils.rs`에 비즈니스 로직 혼재 | 수동 검토 |
...
> **에러 메시지 보안**: 에러의 `#[error("...")]` 메시지는 외부 HTTP 응답에 노출될 수 있다. 내부 구현 정보(DB 쿼리, 파일 경로)는 포함하지 않는다 (`security.md §에러 응답` 참조).

---

### [R-R-03] Newtype + Smart Constructor — 도메인 개념 타입화

**적용 시점**: 동일 타입의 파라미터가 여러 개 나열되거나, primitive 타입이 도메인 개념을 대신할 때
...
// ✅ After: Newtype + Smart Constructor로 도메인 규칙 강제

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct AccountId(u64);

impl AccountId {
    /// Smart Constructor: 유효한 계좌 ID만 생성 허용
...
impl AmountCents {
    /// Smart Constructor: 비즈니스 규칙(금액은 0 이상)을 생성 시점에 강제
...
> **도메인 원칙**: Newtype 단독으로는 불충분하다. Smart Constructor(`new` → `Result`)로 객체 생성 시점에 invariant를 보장해야 한다. 외부에서 내부 값을 직접 생성하지 못하도록 필드를 `pub(crate)` 또는 private으로 유지한다.

---

### [R-R-04] Repository Trait 추상화 — 도메인과 인프라 분리

**적용 시점**: 도메인 로직이 DB 구체 타입(`sqlx::PgPool`)에 직접 의존할 때

```rust
// ❌ Before: 도메인 서비스가 인프라 구현체에 직접 의존
pub struct OrderService {
    db: sqlx::PgPool,  // 도메인이 인프라에 묶임
}

// ✅ After: Trait으로 의존성 역전 (Dependency Inversion)

// 1. 도메인 레이어: Repository Trait 정의 (인프라 의존 없음)
...
// 4. 통합 테스트: 실제 DB 사용 (Repository를 모킹하지 않는다)
#[cfg(test)]
mod tests {
    use super::*;
    use sqlx::PgPool;
    ...
    #[sqlx::test]
    async fn get_order_returns_not_found_when_missing(pool: PgPool) {
        let repo = PostgresOrderRepository { pool };
        let service = OrderService::new(repo);

        let result = service.get_order(OrderId::new(999).unwrap()).await;
...
> **테스트 원칙**: Repository 구현체는 실제 DB로 테스트한다. `mockall`로 Repository를 모킹하면 실제 SQL 오류, 트랜잭션, 인덱스 동작을 검증할 수 없다 (`test.md` Mocking Rules 참조). 외부 결제 API처럼 제어 불가능한 시스템 경계에서만 모킹을 사용한다.
...
fn fetch_user_profile(id: u64) -> Result<UserProfile, reqwest::Error> {
...
// 1. 공유 HTTP 클라이언트 (앱 초기화 시 한 번만 생성)
...
impl HttpClient {
    ...
    // 2. 비동기 개별 요청
    pub async fn fetch_user_profile(&self, id: u64) -> Result<UserProfile, reqwest::Error> {
        self.inner
            .get(format!("https://api.example.com/users/{id}"))
            .send()
            .await?
...
async fn process_users(client: &HttpClient, ids: Vec<u64>) -> Vec<UserProfile> {
    let futures = ids.iter().map(|&id| client.fetch_user_profile(id));
...
async fn process_users_limited(
    ...
    let futures = ids.into_iter().map(|id| {
        ...
        async move {
            ...
            client.fetch_user_profile(id).await.ok()
        }
    });
...
```rust
// src/domain/order/mod.rs — 공개 API 제어 (pub 노출 최소화)
mod error;
mod model;
mod repository;
mod service;
...
**적용 시점**: `String`을 `&str` 대신 사용하거나, 라이프타임 오류를 `.clone()`으로 회피할 때
...
fn find_config_value(configs: &[Config], key: &str) -> String {
    configs.iter()
...
refactor(order):   [R-R-03] AccountId, AmountCents Newtype + Smart Constructor 도입
refactor(domain):  [R-R-04] OrderRepository Trait 추상화, 도메인·인프라 분리
...
```
❌ 도메인 없는 util 모듈
   → 도메인 개념이 없다면 추출하지 않는다. utils.rs에 비즈니스 로직을 넣지 않는다.

❌ Primitive 집착 (primitive obsession)
   → u64, String으로 도메인 개념 표현 금지. Newtype + Smart Constructor 사용.

❌ Bool 플래그로 상태 표현
   → is_paid + is_shipped 조합 대신 OrderStatus enum 사용.

❌ 성급한 추상화 (Rule of Three 위반)
   → 동일 패턴이 3번 나타나기 전에 추상화하지 않는다.

❌ unsafe 블록을 "성능"을 이유로 근거 없이 도입
   → 대안: 알고리즘 개선, Rayon 병렬화
...
❌ 에러 타입 없이 Box<dyn Error>로 통일
   → thiserror로 도메인 의미를 가진 구체적 에러 타입 정의

❌ 내부 Repository·UseCase를 mockall로 모킹
   → 실제 DB를 사용하는 통합 테스트로 대체. 모킹은 외부 API 경계에서만.

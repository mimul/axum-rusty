...
description: /address-review 커맨드로 실행되는 리뷰 대응 스킬. 코드 리뷰 결과를 받은 사람이 각 지적 사항의 타당성을 판단하고 수정을 수행하는 데 Claude가 보조하는 스킬이다.
  STEP 2 타당성 평가 시 coding-style.md(도메인 중심 진화형 코딩 원칙)를 1차 판단 기준으로, 보안 정첵·test-style.md를 보완 기준으로 적용한다.
  각 지적의 기술적 타당성과 프로젝트 정책 적합성을 독자적으로 평가하고, 타당한 지적만 Before/After로 수정하며 최종 요약을 출력한다.
---

# `/address-review` 커맨드 스킬

coding-style.md(1차), 보안 정책(아래 후술), test-style.md(보완) 로드 후 지적사항에 대해 타당성을 독립적으로 평가해 해당 지적사항을 수정한다.
...
/address-review --pr 42 --dry-run         PR #42 지적사항 독립적 평가 후 수정사항에 대해 Before/After 포함한 초인만 출력
...
2. `.claude/rules/test-style.md` — 지적의 보완 기준
3. 보안 정책
XX

각 지적에 대해 아래 4가지 기준으로 독립적으로 평가한다.
...
coding-style.md 안티 패턴 요약 섹션도 적극 체크한다:
...
| R-08 테스트 | test-style.md §1~§13 전체 (우선순위 기반) | 🚫 통합 테스트 Mock DB 사용(§4.3) · Assertion 없는 테스트(§13.1) · 비결정적 출력 고정 사용(§10.2) · 이유 없는 `#[ignore]`(§10.3) — ⚠️ 핵심 로직 테스트 없음(§6.3) · `mockall expect` 과다(§13.2) · `Result` 에러 케이스 없음(§6) — 💡 네이밍 템플릿 미준수(§3.2) · AAA 패턴 미준수(§5.1) · `#[should_panic]` 사용(§14) |
...
[A-RV-XX] ✅ 대응
  ...
  보완: [XX 위반 / 보안정책 §섹션 / test-style.md §섹션] (해당 시)
...
📏 보완:   [보안 정책 §섹션 / test-style.md §섹션] (해당 시)
...
PR 모드(`/address-review- [번호]`)에서 수정 커밋이 1건 이상 발생한 경우, 요약 출력 **전에** Claude가 직접 push를 실행한다:
...
- 판단의 우선순위: **coding-style.md(1차) → 보안 정책, test-style.md(보완) → CLAUDE.md 컨벤션**
...
- coding-style.md 원칙 및 보안 정책, `test-style.md` 규칙과 충돌하는 리뷰어 제안은 해당 규칙을 우선한다
- PR 모드에서 모든 수정 커밋 완료 후 **STEP 4에서 자동으로 `git push origin [브랜치명]`을 실행**하고 Push 결과를 요약에 포함한다
- push 완료 후 `/reply-review [PR번호]`로 각 리뷰 코멘트에 대응 회신한다

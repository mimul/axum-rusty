...
async fn insert_todo_stores_and_retrieves_by_id() {
    ...
    let found = uow.todo_repo().get(&inserted.id).await.unwrap();
...
async fn get_todo_with_nonexistent_id_returns_none() {
    ...
    let found = uow.todo_repo().get(&id).await.unwrap();
...
async fn find_todos_without_filter_returns_all_inserted() {
    ...
    uow.todo_repo()
        .insert(NewTodo::new(Id::gen(), "Second Todo".to_string(), "Second Desc".to_string()))
        .await
        .unwrap();

    let found = uow.todo_repo().find(None).await.unwrap();
    assert!(
        found.len() >= 2,
...
async fn update_todo_title_updates_correctly() {
    ...
    let updated = uow.todo_repo().update(update).await.unwrap();
...
async fn upsert_todo_inserts_new_record() {
    ...
    let result = uow.todo_repo().upsert(upsert).await.unwrap();
...
async fn upsert_todo_updates_existing_record() {
    ...
    uow.todo_repo().upsert(first).await.unwrap();

    let status2 = uow.todo_status_repo().get_by_code("working").await.unwrap();
    ...
    let result = uow.todo_repo().upsert(second).await.unwrap();
...
async fn delete_todo_removes_and_returns_deleted_todo() {
    ...
    let deleted = uow.todo_repo().delete(&inserted.id).await.unwrap();
    assert!(deleted.is_some(), "delete should return the removed todo");
    assert_eq!(deleted.unwrap().id.value, id_value);

    let after = uow.todo_repo().get(&inserted.id).await.unwrap();
...
async fn delete_nonexistent_todo_returns_none() {
    ...
    let id: Id<Todo> = Id::gen();
    let result = uow.todo_repo().delete(&id).await.unwrap();
